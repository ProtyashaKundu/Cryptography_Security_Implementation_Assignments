...
int main() 
{
    uint32_t key[8] = {0x03020100, 0x07060504, 0x0b0a0908, 0x0f0e0d0c, 0x13121110, 0x17161514, 0x1b1a1918, 0x1f1e1d1c};// standard reference keystream
    ...
    unsigned int ui;
    uint64_t start_cycles = __rdtscp(&ui);//reads the CPU's 64-bit cycle counter while ensuring all prior instructions have finished, writing the count as the return value and the current CPU core ID into &ui.
    ...
    asm volatile("" : : "g"(keystream) : "memory");//tells the C compiler to insert raw assembly instructions directly into the program.
...
//output
//protyasha@protyashaLinux:~/Documents$ ./chacha20
/*=== ChaCha20 AVX2 Benchmark (Nanoseconds) ===
Total Processed Data : 122 MB
Total Execution Time : 180048231 ns
Time per Iteration   : 180.05 ns
Time per Block (64B) : 90.02 ns
Time per Byte        : 1.4066 ns/byte
Cycles per Byte      : 3.51 cpb
Throughput           : 0.66 GiB/s*/
